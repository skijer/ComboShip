...
extern "C" void gSPDisplayList(Gfx* pkt, Gfx* dl) {
    ...
    if (ResourceMgr_OTRSigCheck(imgData) == 1) {
        ...
        // resolves through OOT's own RM, which misses/aliases the other game's assets and crashes
        // on the empty resource. Emit the raw string as a G_DL_OTR_FILEPATH command so the
        // interpreter's routed handler resolves it against the named game's RM (see libultraship
        // interpreter.cpp routing).
        if (imgData[7] == '@') {
