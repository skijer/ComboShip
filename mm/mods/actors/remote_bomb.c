 ...
 *
 * En_Bom with the fuse taken away and a cyan tint on top. One press of the slate's button pulls a
 * bomb into Link's hands, the next sets it off — still held, or thrown with A/B.
 *
...
extern void func_8082F5FC(Player* this, Actor* actor); // engine's own "start carrying this actor"
...
// EnBom_Init leaves the bomb at scale 0, and the frame that grows it is timer == 67 — which a bomb
// with no fuse never reaches.
#define REMOTE_BOMB_SCALE 0.01f
...
// The actor's own display list run between a grayscale recolour and its reset. The engine's colour
// filter has only white, red and blue, so it cannot do cyan.
...
static s32 RemoteBomb_Lift(PlayState* play, Player* player) {
    ...
    Actor* bomb = Actor_SpawnAsChild(&play->actorCtx, &player->actor, play, ACTOR_EN_BOM, player->actor.world.pos.x,
                                     player->actor.world.pos.y, player->actor.world.pos.z, BOMB_EXPLOSIVE_TYPE_BOMB,
    ...
    Actor_SetScale(bomb, REMOTE_BOMB_SCALE);

    // The engine's own carry hand-off, which is also where a bomb from the bag gets its overhead
    // pose — the slate has no item-action row of its own to take it from.
    func_8082F5FC(player, bomb);
...
// Read by the hook that stops the slate's own button from throwing it.
u8 RemoteBomb_IsHeld(void) {
...
void RemoteBomb_Tick(PlayState* play) {
    ...
    if (sRemoteBomb->params != BOMB_TYPE_BODY) {
        RemoteBomb_Release();
        return;
    }
