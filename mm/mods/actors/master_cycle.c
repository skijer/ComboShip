 ...
 *   player's entire riding stack (Player_Action_52, the mount/dismount handlers, the horse camera,
 *   the seated poses, the bow-from-the-saddle path) reads EnHorse fields by name — riderPos,
 *   animIndex, curFrame, action, stateFlags — and re-implementing all of that inside z_player.c for
 *   a second vehicle would be a fork of the player. So the horse struct stays and the horse
 *   behaviour goes. Link sits on it exactly the way he sits on Epona, in Epona's own riding
 *   animations, driven by the bike's speed.
 *
 *   Unlike the SoH original, this one never touches the horse's SKELETON. MM defers Skin_Init to a
 *   second update function whenever the horse object is already in the scene bank — and we replace
 *   that update, so the skeleton would stay uninitialised in exactly the scenes Epona lives in
 *   (Romani Ranch, Gorman Track). It costs nothing to drop: Player_Action_52 reads only `animIndex`
 *   and `curFrame` off the mount and animates from its own table, so those two are driven by hand
 *   here and the Skin is left alone.
 *
...
// z_en_horse.h and gameplay_keep.h (gEffFire1DL) are already in this TU: z_player.c includes them
// long before it includes custom_items.c, which is what pulls this file in.
#include <math.h>
#include <string.h>
#include "objects/object_horse_link_child/object_horse_link_child.h" // Epona's clips, for their lengths
#include "../items/helpers/combat_helper.h"

// THE RIDING BIT, spelled out. Do not write PLAYER_STATE1_ON_HORSE anywhere in this file.
//
// Mario Mode's sm64_mario.c is #include'd into this same translation unit long before we are
// (z_player.c:62, we arrive at :78) and it does `#define PLAYER_STATE1_ON_HORSE 0u` — "MM has no
// horseback", which is true of Mario and false of everything here — with no #undef after it. The
// readable name is therefore poisoned for the rest of the TU: `stateFlags1 & PLAYER_STATE1_ON_HORSE`
// compiles happily and is ALWAYS false. That is what made the bike unridable in every way at once —
// no throttle, no D-pad claim, the bow still on B and Epona's carrots still on screen — while every
// line of it looked correct. nei_oot_compat.h's alias for the same name is #ifndef-guarded, so it
// loses to the poisoned one and cannot save us either.
#define MC_RIDING_FLAG PLAYER_STATE1_800000
...
// Declared here because MM keeps them out of z_en_horse.h.
...
// Where Link sits. Player_Action_52 places him at riderPos - 27 in Y, so the seat value
...
// What counts as ground the bike can ride, and how far over it will lean to do it. A downward
// raycast happily returns near-vertical polygons, and treating those as hills is what let the bike
// climb the inside of a wall — see the clamp in MasterCycle_Move for the mechanism.
#define MC_FLOOR_MIN_NY 0.5f      // normal.y below this is a wall, not a floor (~60 degrees)
#define MC_MAX_FLOOR_PITCH 0x1C00 // ...and the steepest hill it will actually climb (~39 degrees)
...
//
// Link's pose comes free with it. Player_Action_52 picks his riding animation straight out of the
// horse's `animIndex`, and entry ENHORSE_ANIM_LOW_JUMP of that table is his own horseback jump.
// Setting the index is the whole job.
...
// Ticks the throttle is ignored after taking the saddle, so the A that mounted him does not also
// launch the bike. Roughly the length of Link's climb-on animation at 20 Hz.
#define MC_MOUNT_GRACE 18
...
#define MC_ACTION_PARKED ENHORSE_ACTION_IDLE
#define MC_ACTION_RIDDEN ENHORSE_ACTION_MOUNTED_WALK
...
typedef struct {
    ...
    f32 riderFrame; // how far into Epona's clip Link is; the player copies this into his own

    // The two speed ribbons, one trailing each wheel. MM keeps its TOTAL_EFFECT_COUNT sentinel
    // private to z_effect.c, so the "do we own an effect" answer is carried here instead.
    s32 trailIdx[2];
    u8 trailsOwned;
    u8 trailOn[2];
    ...
    u8 wasRiding;
    s16 mountGrace; // ticks of climb-on left before the throttle is his
    u8 airborne;
    s16 hopTimer;
...
u8 MasterCycle_IsRiding(void) {
    ...
    return (player->stateFlags1 & MC_RIDING_FLAG) && (player->rideActor == sMc.actor);
}
...
static void MasterCycle_Sfx(u16 id) {
    if (sMc.actor != NULL) {
        Audio_PlaySfx_AtPos(&sMc.actor->projectedPos, id);
    }
}
...
static void MasterCycle_TrailsInit(PlayState* play) {
    ...
    init.unkFlag = false;
    init.calcMode = 2;
    // No trailType here: that field is a SoH addition for its cosmetics menu and MM's struct
    // ends at calcMode.
...
static void MasterCycle_TrailsFree(PlayState* play) {
    ...
    for (i = 0; i < 2; i++) {
        Effect_Destroy(play, sMc.trailIdx[i]);
    }
    sMc.trailsOwned = 0;
}
...
//
// riderPos is relative, not absolute: Player_Action_52 places Link at
 ...
 * Player_Action_52 does) turned him by the slide as well as by the corner, so he ended up
...
static Actor* MasterCycle_Spawn(PlayState* play, Player* player, u8 underLink) {
    ...
    // No object juggling here, unlike the SoH version: MM's En_Horse_Profile already declares
    // GAMEPLAY_KEEP, which every scene has, so Actor_Spawn never refuses it.
    ...
    // These params are load-bearing, and they are Horse_SpawnOverworld's own (z_horse.c:146) — the
    // free-roaming ridable Epona. MM's Init picks the horse's TYPE out of the high bits and every
    // other route is a dead end for us: the plain branch, which is where OoT's params 0 would land,
    // sets HORSE_TYPE_EPONA and then Actor_Kill()s itself outright, and the bandit and donkey
    // branches kill themselves unless OBJECT_HA is in the scene bank. ENHORSE_1 is the ridable
    // behaviour with no cutscene, no race and no minigame attached.
    actor = Actor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, pos.x, pos.y, pos.z, 0, yaw, 0,
    ...
    actor->terminalVelocity = MC_MIN_VEL_Y; // MM's name for OoT's minVelocityY
    actor->speed = 0.0f;
...
// used: setting ENHORSE_FLAG_6 makes Player_Action_52 play its own dismount, so Link never
...
// ── Per-frame tick, from the slate (runs inside Player_UpdateCommon) ──────────
...
/**
 * Offer the bike as the thing A will mount.
 *
 * This CANNOT live in the bike's own update, which is where the SoH original has it. Both the bike
 * and MM's real Epona are ACTORCAT_BG and both write `player->rideActor` every frame; whoever runs
 * later in that category's list wins, and the order is just how the list was built. We lost, so A
 * mounted HER — which is what "A does not accelerate, and letting go walks me backwards" was: her
 * idle AI, on her, with the bike parked somewhere behind.
 *
 * Called from the tick instead, which runs from Player_UpdateCommon (z_player.c:13387) — after
 * every ACTORCAT_BG actor has had its say and ~300 lines before Player_ActionHandler_3 reads the
 * field. So the claim is the last word every frame, whatever the actor order happens to be.
 *
 * HUMAN ONLY, and that is MM's rule, not ours: its mount handler refuses a non-human form and makes
 * the mount TALK instead, so offering the bike to a Deku would earn Link a line of Epona's dialogue
 * and nothing else. And it never steals him off a mount he is already sitting on.
 */
static void MasterCycle_OfferMount(PlayState* play, Player* player) {
    Actor* actor = sMc.actor;
    f32 dx;
    f32 dy;
    f32 dz;
    f32 dist;
    ...
    if ((actor == NULL) || (sMc.phase != MC_PHASE_ACTIVE) || (fabsf(sMc.speed) >= MC_STOPPED) ||
        (player->transformation != PLAYER_FORM_HUMAN) || (player->stateFlags1 & MC_RIDING_FLAG)) {
        return;
    }
    ...
    if ((dist >= MC_MOUNT_RANGE) || (fabsf(dy) >= MC_MOUNT_MAX_DY)) {
        return;
    }

    // Which side he is on, in the bike's own frame: sign of his local X.
    localX = (dx * Math_CosS(actor->shape.rot.y)) - (dz * Math_SinS(actor->shape.rot.y));
    sMc.mountSide = (localX >= 0.0f) ? 1 : -1;
    Actor_SetRideActor(play, actor, sMc.mountSide);
}
...
void MasterCycle_Tick(PlayState* play, Player* player) {
    ...
    if (sLastScene != play->sceneId) {
        ...
        // Arriving on the bike. This is Horse_Spawn's job done for the bike, and done for ANY scene
        // rather than the handful Epona is allowed in: put it under Link, formed, and hand him to
        // it. The player's own resume path then does the rest — the next update sees him riding
        // with no parent and drops straight into the seated pose, exactly as it does for Epona
        // after a transition. Player_MountHorse also sets CAM_SET_HORSE itself, so unlike the SoH
        // version there is no separate camera request to make.
        if (sMcCarry) {
            Actor* bike;
            ...
            if (bike != NULL) {
                Player_MountHorse(play, player, bike);
    ...
    MasterCycle_OfferMount(play, player);

    // Riding into a loading zone. Two things happen during the fade:
    //   - The player sets gHorseIsMounted when he leaves a scene riding, and Horse_Spawn reads it
    //     on the far side to put EPONA under him — which would turn the bike into a horse. Cleared
    //     every frame of the fade. (SoH's equivalent is the nameless AREG(6).)
    ...
    if ((sMc.wasRiding == 2) && !MasterCycle_IsRiding() && !(player->stateFlags1 & MC_RIDING_FLAG)) {
        sMc.wasRiding = 0;
        MasterCycle_BeginDismiss();
    }
}
    ...
    for (i = 0; i < 2; i++) {
        ...
        }
        {
            ...
            if (over > 0.0f) {
                ...
                actor->wallYaw = Math_Atan2S_XY(nz, nx); // _XY: MM's arg order is the reverse of OoT's
...
static void MasterCycle_Move(PlayState* play, Actor* actor) {
    ...
    if (!sMc.airborne && (actor->floorPoly != NULL)) {
        ...
        if (ny > MC_FLOOR_MIN_NY) {
            // Uphill along the direction of travel: rise per unit run is -(n.d)/ny.
            //
            // Math_Atan2S_XY, NOT Math_Atan2S: MM takes (y, x) where OoT takes (x, y), and MM keeps
            // the OoT order under the _XY name. Ported straight, `Math_Atan2S(1.0f, slope)` asked
            // for atan(1/slope) instead of atan(slope) — so FLAT ground came out as 90 degrees, and
            // from there `speed * cos(90)` is zero (A moved nothing at all) while the slope term
            // subtracted sin(90)*g every tick (letting go rolled it backwards). Both halves of that
            // bug were this one swapped pair.
            s16 want = Math_Atan2S_XY(1.0f, (-(nx * sn) - (nz * cs)) / ny);

            if (sMc.speed < 0.0f) {
                want = -want; // reversing up the same slope is going downhill
            }
            // CLAMPED, and this is what stops a steep face being driven through. The split below
            // sends cos(pitch) of the speed along the ground and sin(pitch) of it upward; let the
            // pitch approach vertical and the horizontal part goes to nothing while the vertical
            // part takes everything. A body with no XZ movement gives the wall check nothing to
            // test against — Actor_UpdateBgCheckInfo looks along the line from prevPos to pos — so
            // the bike slid straight up the inside of the geometry. A motorcycle has a limit to
            // what it will climb; past that the slope is a wall, and it is left to hit it.
            if (want > MC_MAX_FLOOR_PITCH) {
                want = MC_MAX_FLOOR_PITCH;
            } else if (want < -MC_MAX_FLOOR_PITCH) {
                want = -MC_MAX_FLOOR_PITCH;
            }
            Math_SmoothStepToS(&sMc.floorPitch, want, 2, 0x600, 0x40);
        } else {
            // Too steep to be a floor at all: no slope model, so the speed stays horizontal and
            // runs into the face instead of riding up it.
            Math_SmoothStepToS(&sMc.floorPitch, 0, 2, 0x600, 0x40);
        }
    } else {
    ...
    // "Never hug small drops". func_800B7678 keeps an actor glued to the floor for any gap under 11
    // units — which is most of a hop's first frames and the whole lip of a ramp — UNLESS it carries
    // this flag. It is the Goron roll's own exemption, and a bike wants exactly the same deal:
    // leave the ground the instant the ground leaves, so ledges launch instead of sticking.
    actor->bgCheckFlags |= BGCHECKFLAG_PLAYER_800;

    actor->speed = sMc.speed * Math_CosS(sMc.floorPitch);
...
static void MasterCycle_Drive(PlayState* play, Actor* actor, Player* player) {
    ...
    // Talking, cutscenes and the like: coast to a stop, no input. Player_InCsMode carries the
    // cutscene half on its own here — MM's bit 29 is "time stopped, animations continue", which
    // also fires on the ocarina and the pause menu, so testing it would stall the bike for those.
    if (Player_InCsMode(play) || (player->stateFlags1 & PLAYER_STATE1_TALKING)) {
        held = 0;
        pressed = 0;
        stickX = 0.0f;
        stickMag = 0.0f;
...
/**
 * Which of Epona's poses Link should be sitting in, from the speed, and how far into it he is.
 *
 * Player_Action_52 reads exactly two things off the mount: `animIndex`, to pick the rider clip out
 * of its own table, and `curFrame`, which it writes straight into Link's skelAnime. So those two
 * are all that has to be true here — and driving them by hand is why this file never touches the
 * horse's Skin. See the note at the top: MM leaves that Skin uninitialised in the very scenes Epona
 * lives in, and calling SkelAnime_Update on it there would be a crash.
 *
 * The frame is wrapped on the EPONA clip's own length, which keeps it in the range the player's
 * animation was authored against — the two are frame-synced by design, which is the whole reason
 * vanilla can copy one into the other.
 */
static void MasterCycle_UpdateRiderPose(EnHorse* horse) {
    ...
    if (sMc.airborne) {
        ...
        anim = object_horse_link_child_Anim_0035B0;
        playSpeed = 1.0f;
        ...
        anim = gEponaGallopAnim;
        playSpeed = 0.9f + ((sp - (MC_MAX_SPEED * 0.66f)) / (MC_MAX_SPEED * 1.3f));
    }

    if (horse->animIndex != want) {
        horse->animIndex = want;
        sMc.riderFrame = 0.0f;
    }

    last = (f32)Animation_GetLastFrame((void*)anim);
    sMc.riderFrame += playSpeed;
    while ((last > 0.0f) && (sMc.riderFrame > last)) {
        sMc.riderFrame -= last;
    }
    horse->curFrame = sMc.riderFrame;
}
...
static void MasterCycle_UpdateHitCollider(PlayState* play, Actor* actor) {
    ...
    // MM has the exact flag for this and OoT does not: being run over by a horse. A bike at speed
    // is the same event as far as anything it hits is concerned.
    cfg.dmgFlags = DMG_HORSE_TRAMPLE;
    cfg.damage = dmg;
    cfg.effect = 0;
...
static void MasterCycle_Update(Actor* thisx, PlayState* play) {
    ...
    if (sMc.phase == MC_PHASE_DISMISS) {
        ...
        if (--sMc.phaseTimer <= 0) {
            thisx->bgCheckFlags &= ~BGCHECKFLAG_PLAYER_800; // ours only; not for a recycled actor
    ...
    if (riding && !sMc.wasRiding) {
        sMc.mountGrace = MC_MOUNT_GRACE;
    }
    if (!riding) {
        sMc.mountGrace = 0;
    }

    // The offer to mount is NOT made here — see MasterCycle_OfferMount, which runs from the tick
    // inside the player's own update. Losing that race to the real Epona is what put Link on her.
    ...
    } else if (riding && (sMc.mountGrace == 0)) {
        // SEATED. The job of this test is only to keep the A that Link is holding through the
        // climb-on animation from driving the bike out from under him — so it is a timer we own,
        // not a state we read off the player.
        //
        // It used to read `player->av2.actionVar2 != 0`, which is what OoT means by "done mounting"
        // and is NOT what MM means. av2 is union scratch shared between action funcs, and the log
        // caught it flickering 4 → 1 → 0 while the rider was demonstrably seated and rolling
        // (same=1 ridingBit=1 action=MOUNTED_WALK animIdx=WALK). Every tick it read 0 the bike fell
        // into the "still climbing on" branch below and had its speed slammed to zero — which is
        // exactly "it will not go forwards", and the leftover slope term walking it backwards.
    ...
    if ((thisx->bgCheckFlags & BGCHECKFLAG_WATER) && (thisx->depthInWater > MC_DEEP_WATER)) {
        sMc.speed = 0.0f;
        if (riding) {
            MasterCycle_ForceDismount();
        }
    ...
    // So does putting on a mask. The C buttons still work up here, so he can transform mid-ride, and
    // the seated poses are the human skeleton's — a Goron in that saddle has no animation to be in.
    if (riding && (player->transformation != PLAYER_FORM_HUMAN)) {
        MasterCycle_ForceDismount();
    }
    ...
    // (Player_Action_52: shape.rot.y = rideActor's), so the lean and the wheelie pitch are
    ...
    Collider_UpdateCylinder(thisx, &horse->colliderCylinder1);
    Collider_UpdateCylinder(thisx, &horse->colliderCylinder2);
    horse->colliderCylinder1.dim.pos.x += (s16)(Math_SinS(thisx->shape.rot.y) * 22.0f);
    horse->colliderCylinder1.dim.pos.z += (s16)(Math_CosS(thisx->shape.rot.y) * 22.0f);
    horse->colliderCylinder2.dim.pos.x += (s16)(Math_SinS(thisx->shape.rot.y) * -24.0f);
    horse->colliderCylinder2.dim.pos.z += (s16)(Math_CosS(thisx->shape.rot.y) * -24.0f);
    CollisionCheck_SetOC(play, &play->colChkCtx, &horse->colliderCylinder1.base);
    CollisionCheck_SetOC(play, &play->colChkCtx, &horse->colliderCylinder2.base);
    if (riding) {
    ...
    thisx->speed = fabsf(sMc.speed); // what the player-side code reads to gate talking etc.
}
...
static void MasterCycle_DrawChargeFlames(PlayState* play, Actor* actor) {
    ...
    for (i = 0; i < MC_MT_FLAME_COUNT; i++) {
        ...
        Matrix_RotateYS((s16)((Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) - actor->shape.rot.y) + 0x8000),
                        MTXMODE_APPLY);
...
static void MasterCycle_Draw(Actor* thisx, PlayState* play) {
    ...
    if (sMcDlWheelB != NULL) {
        Matrix_Push();
        Matrix_Translate(0.0f, MC_HUB_Y, MC_HUB_B_Z, MTXMODE_APPLY);
        Matrix_RotateXF(sMc.wheelSpin, MTXMODE_APPLY);
        MATRIX_FINALIZE_AND_LOAD(POLY_OPA_DISP++, play->state.gfxCtx);
    ...
    if (sMcDlWheelF != NULL) {
        ...
        Matrix_RotateYS(sMc.steer, MTXMODE_APPLY);
        Matrix_RotateXF(sMc.wheelSpin, MTXMODE_APPLY);
        MATRIX_FINALIZE_AND_LOAD(POLY_OPA_DISP++, play->state.gfxCtx);
