 ...
 * scratchpad/port_stasis_to_mm.py, so edit THAT copy. Beyond the usual renames (MM's actor table
 * is its own, `speed` replaces `speed`, the scene is `sceneId`, the camera list is `bgCamList`)
 * there are three real divergences, each marked "MM:" where it happens:
 *   - the instance size comes from gActorOverlayTable, MM having no ActorDB;
 *   - "keeps its pose" is a local list, 2ship's Ultrahand being an older build with no trait table
 *     to share;
 *   - the carried-flame pass is absent for the same reason. See Stasis_KeepsItsPose.
 */
...
// whatever their own logic wanted, and Actor_UpdateVelocityWithGravity clamps to it every frame — a
...
static const s16 sStasisPropIds[] = {
    ...
    // Rolling boulders. Catching one in mid-roll and sending it back the way it came is the
    // single most Stasis-shaped thing in either game.
    ACTOR_EN_GOROIWA,
    ...
    // a lit bomb out of time, charge it up, and fire it where you want it.
    ACTOR_EN_BOM,
    ACTOR_EN_BOMBF,
    ACTOR_EN_BOM_CHU,
};
...
//
// MM: soh reads this off Ultrahand's PACCI_UH_TRAIT_EXCLUDE rows, which 2ship's older Ultrahand does
// not have. Rather than start a second list that would drift from soh's, the answer here is the one
// property that actually separates a body from a room: SIZE. A background actor taller than a
// four-storey drop is not a thing you pick up, it is the place you are standing in — Stone Tower's
// rotating rooms, the moving parts of Great Bay, the pirate boat. Bodies stay well under it (the
// tallest tree measures 480 and is ACTORCAT_PROP anyway).
#define STASIS_STRUCTURE_HEIGHT 700.0f

static u8 Stasis_IsStructure(Actor* actor) {
    if (actor->category != ACTORCAT_BG) {
        return 0;
    }
    return Stasis_MeasureHeight(gPlayState, actor) >= STASIS_STRUCTURE_HEIGHT;
}
...
static u8 Stasis_NpcIsSafeToFreeze(PlayState* play, Actor* actor) {
    ...
    if (Player_InCsMode(play) || (CutsceneManager_GetCurrentCsId() != CS_ID_NONE)) {
        return 0;
    }
...
// ACTORCAT_EXPLOSIVES is not decoration here: En_Bom and friends have been on the prop list for
...
static const u8 sStasisCats[6] = {
    ACTORCAT_ENEMY, ACTORCAT_PROP, ACTORCAT_BG, ACTORCAT_NPC, ACTORCAT_EXPLOSIVES, ACTORCAT_MISC,
};
...
// the actor's real allocated size comes from its overlay entry, so the scan never runs off the end.
...
static s32 Stasis_ScanOwnColliders(Actor* actor, ColliderCylinder** out, s32 max) {
    // MM: no ActorDB. The overlay table carries the same number, one hop further in — and the
    // profile pointer is NULL for an overlay that is not currently loaded, which for a frozen
    // actor cannot happen but is worth not assuming.
    ActorOverlay* overlay = ((actor->id >= 0) && (actor->id < ACTOR_ID_MAX)) ? &gActorOverlayTable[actor->id] : NULL;
    size_t size = ((overlay != NULL) && (overlay->profile != NULL)) ? overlay->profile->instanceSize : 0;
    u8* base = (u8*)actor;
    size_t off;
    s32 count = 0;
...
// carry the right colMaterial and AC_HARD, so a sword on a frozen boulder gives the vanilla clonk and
...
//
// MM: soh reads this off Ultrahand's PacciUhTraitRow table (the NO_TURN / NO_FACE rows), which
// 2ship's older Ultrahand does not have. The list below is that answer written out locally. If
// 2ship's cane ever grows the trait table, delete this and read it there instead — one list beats
// two, and two lists is exactly how the games drift apart.
static const s16 sStasisKeepPoseIds[] = {
    ACTOR_OBJ_OSHIHIKI, ACTOR_OBJ_ICEBLOCK, ACTOR_BG_ICEFLOE, ACTOR_OBJ_ARMOS, ACTOR_OBJ_LIFT,
};
...
static u8 Stasis_KeepsItsPose(Actor* actor) {
    ...
    return Stasis_IdInList(actor->id, sStasisKeepPoseIds, ARRAY_COUNT(sStasisKeepPoseIds));
}

// MM: soh gives a frozen torch an open-flame collider, driven by Ultrahand's BURNS rows and by
// its Pacci_UhTorchLit condition. Neither exists here — 2ship's Ultrahand has no trait table, and
// whether an Obj_Syokudai is LIT lives in its private struct (`snuffTimer`), which a mod cannot
// include a header for. Guessing that wrong means a frozen UNLIT torch setting the room on fire,
// so the pass is left out rather than approximated, and the torches are absent from the whitelist
// to match. Port it properly once 2ship's cane grows the trait table.
...
static ColliderElement sStasisFakeAtInfo;
...
static void Stasis_DeliverStoredHit(PlayState* play, Actor* actor) {
    ...
    sStasisFakeAtInfo.atDmgInfo.dmgFlags = sStasis.lastDmgFlags;
    sStasisFakeAtInfo.atDmgInfo.effect = sStasis.lastDmgEffect;
    sStasisFakeAtInfo.atDmgInfo.damage = sStasis.lastDmgAmount;
    sStasisFakeAtInfo.atElemFlags = ATELEM_ON;
    ...
    target->elem.acHit = &sStasisFakeAtCollider;
    target->elem.acHitElem = &sStasisFakeAtInfo;
    target->elem.acElemFlags |= ACELEM_HIT;
    target->elem.acDmgInfo.hitPos.x = (s16)actor->world.pos.x;
    target->elem.acDmgInfo.hitPos.y = (s16)actor->world.pos.y;
    target->elem.acDmgInfo.hitPos.z = (s16)actor->world.pos.z;

    actor->colChkInfo.damage = 0;
    CollisionCheck_ApplyDamage(play, &play->colChkCtx, &target->base, &target->elem);
...
static void Stasis_End(PlayState* play) {
    ...
    // exactly as the Pacci lift does with a thrown object.
    {
        ...
        // Full 3D: the horizontal part of the blow becomes speed along its yaw, the vertical part
        ...
        // world.rot.y is the direction of TRAVEL — Actor_MoveWithGravity derives velocity from it, so
...
static void Stasis_Begin(PlayState* play, Actor* target, u8 kind, s32 bgId) {
    ...
    // generic prop, and it should bounce the sword the same way the trunk does.
    {
        ...
        if (sStasis.ownColliderCount > 0) {
            sStasis.collider.base.colMaterial = sStasis.ownCollider[0]->base.colMaterial;
...
static void Stasis_CaptureHit(PlayState* play, Actor* actor) {
    ...
    if (hitCollider != NULL) {
        ...
        if (len > 1.0f) {
            ...
            contact.x = (f32)hitCollider->elem.acDmgInfo.hitPos.x;
            contact.y = (f32)hitCollider->elem.acDmgInfo.hitPos.y;
            contact.z = (f32)hitCollider->elem.acDmgInfo.hitPos.z;
        ...
        // ColliderElement for a ten-second stasis is a dangling read. See Stasis_DeliverStoredHit.
        if (hitCollider->elem.acHitElem != NULL) {
            u32 flags = hitCollider->elem.acHitElem->atDmgInfo.dmgFlags;
            ...
            if (flags != 0) {
                ...
                sStasis.lastDmgEffect = hitCollider->elem.acHitElem->atDmgInfo.effect;
                sStasis.lastDmgAmount = hitCollider->elem.acHitElem->atDmgInfo.damage;
            }
        }
...
void Stasis_Update(PlayState* play, Player* player) {
    ...
    if (sStasis.phase == STASIS_PHASE_FLYING) {
        ...
        if (sStasis.slideLaunch) {
            ...
            actor->speed *= STASIS_SLIDE_FRICTION;
            if ((actor->bgCheckFlags & BGCHECKFLAG_WALL) || (actor->speed < 1.0f) || (sStasis.timer <= 0) ||
                (sStasis.collider.base.atFlags & AT_HIT)) {
                Actor_PlaySfx(actor, NA_SE_EV_BLOCK_BOUND);
                Stasis_RestoreActor();
                Stasis_Forget();
            }
            return;
