...
// Rando.cpp: the hint is built and cached there, then shown through 2ship's CustomMessage system.
// PrepareHint returning 0 also covers "this is not a rando save" — no wish can be answered there.
extern u8 SensorRune_PrepareHint(void);
extern void SensorRune_OpenPrompt(void);
extern void SensorRune_OpenHint(void);
...
static void Sensor_PayHeartContainer(Player* player) {
    ...
    data->healthCapacity -= SENSOR_HEART_COST;
    if (data->health > data->healthCapacity) {
    ...
    func_800AA000(200.0f, 180, 20, 40);
}
...
s32 Sensor_Cast(PlayState* play, Player* player) {
    ...
    if (gSaveContext.save.saveInfo.playerData.healthCapacity < (SENSOR_CAPACITY_FLOOR + SENSOR_HEART_COST)) {
        return 0;
    }
    if (!SensorRune_PrepareHint()) {
        return 0;
    }

    Sensor_HoldPlayer(player);
    SensorRune_OpenPrompt();
