 ...
 * update is NOT reused: ObjLift's wait shakes and its fall drops the slab and sets a scene switch
 * flag on landing, and neither belongs to a summon. Its destroy IS kept — that unregisters the
 * collision.
 */
...
static void WandSand_Measure(PlayState* play, Actor* slab) {
    ...
    CollisionHeader* header = play->colCtx.dyna.bgActors[bgId].colHeader;
    if (header == NULL) {
        return;
    }

    f32 halfX = (f32)(header->maxBounds.x - header->minBounds.x) * 0.5f * SAND_SLAB_SCALE;
    f32 halfZ = (f32)(header->maxBounds.z - header->minBounds.z) * 0.5f * SAND_SLAB_SCALE;
...
static u8 WandSand_Covers(Vec3f* spot) {
    for (u8 i = 0; i < SAND_MAX_SLABS; i++) {
        ...
        f32 dx = slab->world.pos.x - spot->x;
        f32 dz = slab->world.pos.z - spot->z;
 ...
 * ObjLift's destroy takes the collision at exactly the moment the model stops being drawn.
 */
