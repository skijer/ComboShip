 ...
 *
 * Ported from soh by scratchpad/port_stasis_to_mm.py — do not hand-edit, edit the soh copy.
 *
...
static const s16 sSwitchMagnetPressers[] = {
    ...
    ACTOR_OBJ_LIFT,     // platform slab
    ACTOR_OBJ_KIBAKO2,  // large crate
    ACTOR_OBJ_ICEBLOCK,    ACTOR_OBJ_HSBLOCK, ACTOR_OBJ_HAMISHI, ACTOR_OBJ_BOMBIWA,
    ACTOR_OBJ_HUGEBOMBIWA, ACTOR_EN_ISHI,     ACTOR_EN_WOOD02,   ACTOR_OBJ_ARMOS,
};
...
//
// Obj_Switch packs its kind into params exactly as it does in OoT: type in bits 0..2, subtype in
// bits 4..6 (OBJ_SWITCH_GET_TYPE / _SUBTYPE). MM has SIX types rather than five, and two of them
// run the weight path — FLOOR (0) and FLOOR_LARGE (5); the rusty one is an unused stub here, and
// the eye and crystal ones want a projectile.
//
// The subtype enum is MM's own and does not line up with OoT's by name, only by number.
// OBJSWITCH_SUBTYPE_RESET is 2 — "set switch flag when pressed, unset when released", which is the
// weight-driven blue plate and the only one worth aiming at. RESET_INVERTED (3) also goes through
// DynaPolyActor_IsSwitchPressed but clears its flag when pressed, so landing on one would undo a
// puzzle rather than solve it: left alone, same as in soh.
//
...
s32 SwitchMagnet_Steer(PlayState* play, Actor* actor) {
    ...
    for (it = play->actorCtx.actorLists[ACTORCAT_SWITCH].first; it != NULL; it = it->next) {
        f32 dx;
        f32 dz;
        f32 dist;
        f32 t;
    ...
    Math_StepToF(&actor->speed, needSpeed, SWITCHMAGNET_SPEED_STEP);
    return 1;
}
...
void SwitchMagnet_PressUnder(PlayState* play, Actor* actor, f32 halfWidth, f32 bottomY) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        floorY = BgCheck_EntityRaycastFloor5(&play->colCtx, &poly, &bgId, actor, &probe);
