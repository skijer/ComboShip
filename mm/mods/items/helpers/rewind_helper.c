...
// half of what the player actually watches move. MM: OoT spells it ACTORCAT_EXPLOSIVE.
static const u8 sRewindCats[] = {
    ACTORCAT_SWITCH, ACTORCAT_BG,         ACTORCAT_EXPLOSIVES, ACTORCAT_NPC,  ACTORCAT_ENEMY,
...
static size_t Rewind_GetInstanceSize(Actor* actor) {
    ActorOverlay* overlay = ((actor->id >= 0) && (actor->id < ACTOR_ID_MAX)) ? &gActorOverlayTable[actor->id] : NULL;

    return ((overlay != NULL) && (overlay->profile != NULL)) ? overlay->profile->instanceSize : 0;
}
